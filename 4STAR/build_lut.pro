...
; modified to use 4star spectra 
; for arctic experiment
...
if win then restore, 'C:\Users\Samuel\Research\4STAR\model\arctic\sp_'+label+'.out' else $
restore, '/argus/roof/SSFR3/model/sp_'+label+'.out'
;stop
...
if 0 then begin
  ...
  for z=0,n_elements(zenlambda)-1 do $
    for w=0,1 do $
      for r=0, n_elements(ref)-1 do sp_hit[*,r,w,z]=interpol(sp[*,r,0,z,w],tau,tau_hires,/nan)
...
if win then fn='C:\Users\Samuel\Research\4STAR\model\arctic\Pars_lut_'+label+'.out' else $
