...
utc=utc[0:dim-1] & alt=alt[0:dim-1] & lat=lat[0:dim-1] & lon=lon[0:dim-1] & pit=pit[0:dim-1] & rol=rol[0:dim-1] & hed=hed[0:dim-1]
  print,'After nav read: #utc = ',n_elements(utc)
...
if n_elements(utcin) gt 0 then begin
  ...
  noaa_raw_utc = utc
  save,noaa_raw_utc,file=dir+date+'/utc_raw_noaa.sav'
  print,'NOAA raw UTC saved in '+dir+date+'/utc_raw_noaa.sav'
endif else begin
  utcin=utc
endelse

  print,'At end of nav_noaa: #utc = ',n_elements(utc)
end
