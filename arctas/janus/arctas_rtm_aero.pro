...
;   this is the version that is dedicated for use on janus
;
; CATEGORY:
;   ARCTAS, Aerosol
;
...
;   - cluster:      sets paths to the cluster's paths
;   - abs_min:      makes the retrieval find the absolute minimum between asy and asy2
;
; DEPENDENCIES:
;   
;   
...
; Modified: February 1st, 2012 by Samuel LeBlanc
;           - added the cluster keyword for setting paths to the cluster
;           - added the abs_min keyword which sets the retrieval to retrieve the tau that gets the absolute minimum value between asy and asy2
...
pro arctas_rtm_aero, datein, second=second, save=save, plotting=plotting, video=video, divergence=divergence,$
 ...
 cluster=cluster, abs_min=abs_min
...
if cluster then begin
  arctas_dir='/projects/leblanse/arctas/'
  arctas_dir_in='/projects/leblanse/arctas/'
  ll='/'
  dir='/scratch/stmp00/leblanse/libradtran/input/aero/'
  dirout='/scratch/stmp00/leblanse/libradtran/output/aero/'
  aats_dir=arctas_dir_in+'nasa'+ll
endif
...
;stop
corr_eps=0.85 ; suitable correction factors
...
;stop  
  ...
  if keyword_set(angstrom) then file_n=file_n+'angstrom_'
  if error_loop then file_n=file_n+'error_notau_'
  if tau_approx then file_n=file_n+'tauapprox_'
  if hsrl then file_n=file_n+'hsrl_'
  if hist then file_n=file_n+'hist_'
  ...
  for i=fl_s, fl_f do begin ;lon loop
    ...
    tau_out='aero'+string(lambda[iw],format='(I04)')+'.level'
    if not finite(tau[index,aero_wl]) then begin
      ...
      write_input_aats, tau[index,aero_wl], topt, bottomt, tau_aats[near_leg2[index],aero_wl] ,$
       atm_in=atm_in, index=index, tau_out=tau_out, /quiet, tau_approx=tau_approx, dirin=dir
    input_file=dir+'aero_'+date+secon+'_'+string(lambda[iw],format='(I04)')+'.inp'
    output_file=dirout+'aero_'+date+secon+'_'+string(lambda[iw],format='(I04)')+'.out'
...
;;; start tau loop!
    ...
    if abs(asy-asy2) gt 0.5 and not abs_min then begin
	  ...
	  outside:
	  if n_elements(count_asy) eq 0 then count_asy=31
    if n_elements(count_asy2) eq 0 then count_asy2=31
    if tau_mod eq 1.0 then begin    ;save first values, for when the tau retrieval fails
      asy_t1=asy & asy2_t1=asy2 & ssa_t1=ssa & albedo_t1=albedo
    ...
    if (abs(asy-asy2) lt 0.025) and not keyword_set(sensitivity) and not abs_min and count_ssa lt 31 and count_asy lt 31 then begin
