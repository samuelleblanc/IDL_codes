...
set_plot, 'ps'
 ...
 device, xsize=20, ysize=20
  ...
  plot,zenlambda, sp,title='Parameter 11!CSlope between 550 - 680 nm',psym=-4,xtitle='Wavelength (nm)',ytitle='Derivative',xrange=[400,800],yr=[0,1.0]


nul=min(abs(zenlambda-550.),io)
nul=min(abs(zenlambda-680.),in)

ln=linfit(zenlambda[io:in],sp[io:in])

  oplot, zenlambda[io:in], (ln[0]+ln[1]*zenlambda[io:in]), color=50, psym=-4

  legend,['Spectra','Linear fit'],textcolors=[0,50],box=0,/right
device, /close
spawn,'convert '+fp+'.ps '+fp+'.png'
