...
ref=ref_hires
  ...
  yr=[min(avg[*,*,*,1]),max(avg[*,*,*,1])]

  plot,tau,avg[*,0,0,1],/nodata,ytitle='Parameter 2!CDerivative of normalized radiance at 1200 nm (!9m!Xm!U-1!N)',xtitle='Optical depth',yrange=yr
  rs=[4,14,24,39]
  cl=[50,130,210,250]
  tvlct,180,180,255,51 & tvlct,180,255,219,131 & tvlct,255,238,180,211 & tvlct,255,180,180,251
  clb=[51,131,211,251]
  for ri=0,n_elements(rs)-1 do begin
    polyfill, [tau,reverse(tau)],[avg[*,rs[ri],0,1]+std[*,rs[ri],0,1],reverse(avg[*,rs[ri],0,1]-std[*,rs[ri],0,1])],color=cl[ri]
    oplot, tau, avg[*,rs[ri],0,1],thick=3
    ;oplot, tau, avg[*,rs[ri],0,1], color=cl[ri]   
    polyfill, [tau,reverse(tau)],[avg[*,rs[ri],1,1]+std[*,rs[ri],1,1],reverse(avg[*,rs[ri],1,1]-std[*,rs[ri],1,1])],color=cl[ri]
  ...
  legend, ['Liquid','Ice'],box=0,linestyle=[0,2],pspacing=1.4,position=[80,-4.0],/right
