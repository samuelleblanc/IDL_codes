...
;   restore, dir+'data/Pars_pdf_LUT_summer.out'
if snow then snlb='_snow' else snlb=''
restore, dir+'data/Pars_pdf_LUT'+snlb+'_test.out'
...
for i=0, num-1,di do begin
  ...
  for p=0, npar-1 do begin
    if 1 then begin
      mu=par[p]
      if p eq 0 then sig=par[p]*0.08 else sig=par[p]*0.002 
...
;  if model then fn=dir+'data/retrieved_pdf_'+date+'_model.out' else $
   fn=dir+'data/retrieved_pdf_'+date+'_pat.out'
