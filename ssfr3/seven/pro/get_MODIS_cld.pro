...
f=file_search(dir+'MOD03*.hdf')
f=f[sort(f)]
nf=n_elements(f)
f2=file_search(dir+'MOD06*.hdf')
f2=f2[sort(f2)]
...
for i=0, nf-1 do begin
   ...
   if nula lt 2. then begin
     ...
     taus[i]=nan & refs[i]=nans & utaus[i]=nan & urefs[i]=nan & phas[i]=nan & muls[i]=nan & tops[i]=nan & phairs[i]=nan
   endelse
   print,taus[i],refs[i],utaus[i],urefs[i],phas[i],muls[i],tops[i],phairs[i]
endfor
Obj_Destroy, obj

save, f2,taus,refs,utaus,urefs,phas,muls,tops,phairs,filename=dir+'Terra_cld.out'
stop
end
