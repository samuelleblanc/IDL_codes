...
;  tau_rtm1=tau_rtm & ref_rtm1=ref_rtm & wp_rtm1=wp_rtm & tau_rtmi=tau_rtm & ref_rtmi=ref_rtm
  ...
  nani1=where(tau_rtmi le 1. or tau_rtmi ge 198.,ci1)
  if ct1 gt 0 then begin
    tau_rtm1[nans1]=!values.f_nan
    ref_rtm1[nans1]=!values.f_nan
  endif
...
tvlct,100,100,100,201
  ...
  legend,['Multi-parameter(ice)','Multi-patameter(liquid)','Slope','2-wavelengths','Mean','Median'],box=0,pspacing=1,psym=[8,8,8,8,1,6],$
...
if 1 then begin
  fp=dir+'tmhrs_comp_v4_'+lbls[p]+blbl
  print, 'making plot :'+fp
  set_plot, 'ps'
  loadct, 39, /silent
  device, /encapsulated, /tt_font, set_font='Helvetica Bold',/color,bits_per_pixel=8., filename=fp+'.ps'
...
; now make scatter plots comparing each methods for each day
  if cfhm gt 0 then begin
    fp=dir+'scatter_comp_v4_'+lbls[p]+blbl
  print, 'making plot :'+fp
  set_plot, 'ps'
  loadct, 39, /silent
  device, /encapsulated,/tt_font, set_font='Helvetica Bold',/color,bits_per_pixel=8., filename=fp+'.ps'
