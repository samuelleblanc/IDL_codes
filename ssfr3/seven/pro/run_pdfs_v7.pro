...
pro run_pdfs,pars_pdf,bins,taus,ref,wp,meas_pdf,post_tau,post_ref,post_wp,H,H_ind,H_par,tau_err,ref_err,wp_err,post,$
  ic_prob=ic_prob,li_prob=li_prob,ps=ps,post_pdf=post_pdf
...
for pi=0, npar-1 do begin
  ...
  if xplot then begin
    ...
    post_meas[*,*,*,pi]=total(p_likely[*,*,*,p,*],5)
  endif else $
...
;  if not finite(total(post_meas[*,*,*,pi])) then message, 'problem with post meas pdf value'
  ...
  if xplot then begin
    ...
    plot, bins[p,*],pars_pdf[0,0,0,p,*],title='Parameter :'+strtrim(p,2), yrange=[0,0.4],xstyle=0,xr=[bins[p,min(nns)],bins[p,max(nns)]]
    for t=0, 199, 20 do for r=0, 49, 10 do for w=0,1 do oplot, bins[p,*],pars_pdf[t,r,w,p,*]
    oplot, bins[p,*],meas_pdf[p,*],color=250
    for t=0, 199, 10 do for r=0, 49, 5 do for w=0,1 do oplot, bins[p,*],p_likely[t,r,w,p,*], color=70
