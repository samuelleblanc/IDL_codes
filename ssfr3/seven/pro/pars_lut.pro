  ...
  for r=0, n_elements(ref_hires)-1 do begin
    for w=0, n_elements(wp)-1 do begin
      for p=0, n_elements(par_hires[0,0,0,*])-1 do begin
        ...
        avg[t,r,w,p]=par[t,r,w,p,0] ;mean(par[t,r,w,p,*],/nan)
      endfor
    endfor
  endfor
endfor
...
if test eq 1 then fn=dir+'pars_std'+lbsn+'_test.out' else if test eq 2 then fn=dir+'pars_std'+lbsn+'_test2.out' else fn=dir+'pars_std_v3'+lbsn+'.out'
