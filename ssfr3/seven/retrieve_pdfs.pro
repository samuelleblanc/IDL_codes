...
limp=1 ;use the limited p range
...
print, 'restoring data for day: '+date
...
if test eq 1 then restore, dir+'data/Pars_pdf_LUT_summer_test.out' else $
 if test eq 2 then restore, dir+'data/Pars_pdf_LUT_summer_test2.out' else $
  if sno then restore, '/argus/roof/SSFR3/model/Pars_pdf_LUT_snow.out' else $
   restore, '/argus/roof/SSFR3/model/Pars_pdf_LUT_summer.out'

if not limp then npar=n_elements(pars_pdf[0,0,0,*,0]) else npar=n_elements(ps)
...
if test eq 1 then fn=dir+'data/retrieved_pdf_'+date+'_test'+lp+'.out' else $
 if test eq 2 then fn=dir+'data/retrieved_pdf_'+date+'_test2'+lp+'.out' else $
  if model then fn=dir+'data/retrieved_pdf_'+date+'_model'+lp+'.out' else $
   fn=dir+'data/retrieved_pdf_'+date+lp+'.out'
if model then ref=refo  
if model then save, tau_rtm,ref_rtm,wp_rtm,H_rtm,Hi_rtm,Hm_rtm,tau_err,ref_err,wp_err,real,post,spectram,wvl, realm,tau,ref,wp,ic_prob,li_prob,filename=fn else $
save, tmhrs, tau_rtm,ref_rtm,wp_rtm,H_rtm,Hi_rtm,Hm_rtm,tau_err,ref_err,wp_err,real,post,ic_prob,li_prob,filename=fn
