...
print, 'restoring LUT'
if snow then lbsn='_snow' else if not twowvl then lbsn='_summer' else lbsn=''
...
if test eq 2 then label='_test2' else label=''
if test eq 1 then label='_test'
if twowvl then label='_2wvl'
   restore, '/argus/roof/SSFR3/model/Pars_pdf_LUT'+lbsn+label+'.out'

npar=n_elements(pars_pdf[0,0,0,*,0])
...
restore, dir+'data/pars_error.out'
...
for i=0, num-1,di do begin
  if test gt 0 then begin
    ...
    endif else sp=mean(zspectra[*,fl[i:((i+di gt num-1)?num-1:i+di)]],dimension=2)
  endelse
...
;if test eq 1 then fn=dir+'data/retrieved_pdf_'+date+'_test.out' else $
; if test eq 2 then fn=dir+'data/retrieved_pdf_'+date+'_test2.out' else $
;  if model then fn=dir+'data/retrieved_pdf_'+date+'_model.out' else $
   fn=dir+'data/retrieved_pdf_'+date+'_2wvl.out'
;if model then save, tau_rtm,ref_rtm,wp_rtm,H_rtm,Hi_rtm,Hm_rtm,tau_err,ref_err,wp_err,real,post,spectram,wvl, realm,tau,ref,wp,filename=fn else $
save, tmhrs, tau_rtm,ref_rtm,wp_rtm,H_rtm,Hi_rtm,Hm_rtm,tau_err,ref_err,wp_err,real,post,filename=fn
;stop
endfor
stop
end
