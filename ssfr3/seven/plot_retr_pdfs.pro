...
fn='/argus/roof/SSFR3/retrieved/retrieved_pdf_20120523_model_limp_v2.out'
;fn='/home/leblanc/SSFR3/data/retrieved_pdf_20120523_model_limp_v2.out'
restore, fn
hml=hm_rtm


if not win then fn='/argus/roof/SSFR3/retrieved/retrieved_pdf_20120602_model.out' else $
...
if 0 then begin
  ...
  for p=0,n_elements(pi)-1 do begin
    h_mean[p,0,0]=mean(hcl[p,ind[0:12,*,0]],/nan)
    h_mean[p,1,0]=mean(hcl[p,ind[13:21,0:9,0]],/nan)
    h_mean[p,2,0]=mean(hcl[p,ind[13:21,10:*,0]],/nan)
    h_mean[p,0,1]=mean(hcl[p,ind[0:12,*,1]],/nan)
    h_mean[p,1,1]=mean(hcl[p,ind[13:21,0:9,1]],/nan)
    h_mean[p,2,1]=mean(hcl[p,ind[13:21,10:*,1]],/nan)
    
    h_rg[p,0,0,0]=min(hcl[p,ind[0:12,*,0]],/nan)
    h_rg[p,1,0,0]=min(hcl[p,ind[13:21,0:9,0]],/nan)
    h_rg[p,2,0,0]=min(hcl[p,ind[13:21,10:*,0]],/nan)
    h_rg[p,0,1,0]=min(hcl[p,ind[0:12,*,1]],/nan)
    h_rg[p,1,1,0]=min(hcl[p,ind[13:21,0:9,1]],/nan)
    h_rg[p,2,1,0]=min(hcl[p,ind[13:21,10:*,1]],/nan)
    
    h_rg[p,0,0,1]=max(hcl[p,ind[0:12,*,0]],/nan)
    h_rg[p,1,0,1]=max(hcl[p,ind[13:21,0:9,0]],/nan)
    h_rg[p,2,0,1]=max(hcl[p,ind[13:21,10:*,0]],/nan)
    h_rg[p,0,1,1]=max(hcl[p,ind[0:12,*,1]],/nan)
    h_rg[p,1,1,1]=max(hcl[p,ind[13:21,0:9,1]],/nan)
    h_rg[p,2,1,1]=max(hcl[p,ind[13:21,10:*,1]],/nan)
  endfor
...
if 0 then begin
 ...
 hl=fltarr(24,15,2) 
for t=0, 23 do for r=0,14 do for w=0,1 do hl[t,r,w]=hcl[n_elements(hcl[*,0])-1,ind[t,r,w]]
 hlg=hl
...
set_plot, 'ps'
 ...
 ; mu=155B
  mus='!9m!X'
  lvls=findgen(30)/29.
  contour, hlg[0:21,*,0],tau[0:21],ref,levels=lvls,/cell_fill,title='Liquid water cloud',xtitle='Optical depth',$
   ytitle='Effective radius ('+mus+'m)'

plots, [2.,48.,48.,2.,2.],[3.,3.,49.,49.,3.]
plots, [52.,198.,198.,52.,52.],[26.,26.,49.,49.,26.]
plots, [52.,198.,198.,52.,52.],[3.,3.,24.,24.,3.]
xyouts, 25.,25.,'A',alignment=0.5
xyouts, 125.,37.5,'B',alignment=0.5
xyouts, 125.,12.5,'C',alignment=0.5
 
contour, hlg[0:21,*,1],tau[0:21], ref, levels=lvls,/cell_fill,title='Ice water cloud',xtitle='Optical depth',$
...
set_plot, 'ps'
 ...
 device, xsize=40, ysize=20
  ...
  if wp_rtm[ind[ti,ri,wi]] eq 0 then plots, tau_rtm[ind[ti,ri,wi]],ref_rtm[ind[ti,ri,wi]],psym=1, color=255,symsize=3, thick=6
