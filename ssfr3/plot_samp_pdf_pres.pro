...
for uu=0, 12 do begin
fp=dir+'plots\present\pdf\'+string(uu,format='(I02)')
...
set_plot, 'ps'
 ...
 ; !p.multi=[0,3,1]
 !p.multi=0 
 !x.margin=[6,1] & !y.margin=[3.5,1.5] & !y.omargin=[0,0] & & !x.omargin=[0,0.5]
  cl=(findgen(6)+1.)/6.*250.
  ;lvls=(findgen(7)+1.)/7.
  lvls=[0.001,0.01,0.05,0.1,0.2,0.5]
pi=uu
  i=0
  ;postrt[t[0],r[0],w[0],40,6,pi]=postrt[t[0],r[0],w[0],40,6,pi]+0.05
  ;postrt[t[0],r[0],w[0],39,6,pi]=postrt[t[0],r[0],w[0],39,6,pi]-0.05
  if pi gt 9 then postrt[t[0],r[0],w[0],5,14,pi]=0.
  po=reform(postrt[t[0],r[0],w[0],*,0:29,pi])
  if pi eq 1 then po[39,6]=0.0012
  if pi lt 6 then po=smooth(po,2)  
  pu=pi+1
  if pi gt 6 then pu=pu+1
  if pi gt 7 then pu=pu+1
  if pi eq 12 then po[38,7]=0.
  contour, po,taus,refl, xtitle='!9t!X', ytitle='r!De!N (!9m!Xm)',yr=[1,30],xr=[1,100],$
;yr=[6,8],xr=[39,41],$
   levels=lvls,c_color=cl,c_label=cl*0+1,c_charsize=2.0,title='!9h!X!D'+string(pu,format='(I2)')+'!N'
  if pi gt 0 then hhh=total(hprt[t[0],r[0],w[0],0:pi]) else hhh=hprt[t[0],r[0],w[0],0]
  legend,['SIC='+string(hhh,format='(F4.2)')],box=0,/right,charsize=2.0
...
for uu=0, 12 do begin
  fp=dir+'plots\present\sic\'+string(uu,format='(I02)')
  print, 'making plot :'+fp
  set_plot, 'ps'
  loadct, 39, /silent
  device, /encapsulated, /tt_font, set_font='Helvetica Bold',/color,bits_per_pixel=8., filename=fp+'.ps'
...
;  pss=[1,2,3,11,12,13,14]
 ...
 if uu gt 0 then  oplot,ps[0:uu],hpa1[0:uu],color=cl[0],psym=-2,thick=10 else plots,ps[0],hpa1[0],color=cl[0],psym=2
