...
if 1 then begin
  spl=reform(sp[*,0:29,0,*]) & sp_stdl=reform(sp_std[*,0:29,0,*])
  save, spl,sp_stdl,wvl,bmaxl,bminl,nb,nn,binsl,filename=namelut+'_liq.out'
  spi=reform(sp[*,9:*,1,*]) & sp_stdi=reform(sp_std[*,9:*,1,*])
  save, spi,sp_stdi,wvl,bmaxi,bmini,nb,nn,binsi,filename=namelut+'_ice.out'
  stop
endif
...
function pdf_lut_sp, sp,std,wvl,bmax,bmin,bins=bins,nb=nb,nn=nn,tr=tr
...
parr=fltarr(np,nn)
bins=fltarr(np,nb)
if n_elements(tr) lt 1 then tr=[0,nt-1]

for t=tr[0],tr[1] do begin
  for r=0, nr-1 do begin
    for v=0, nv-1 do spp[v,*]=gsim(sp[t,r,v],std[t,r,v]>3E-5,nn)
    for e=0, nn-1 do $ ;begin
       parr[*,e]=get_params(wvl,spp[*,e])
...
; pars, non binned value of parameters, np - number of parameters
; nan where par lut is not valid (either small ref ice, or large ref liquid)
pro get_lut_sav,namelut,tau,refi,refl,pari,parl,bini,binl,np,refs,wps,pars,pexs,pha=pha,para=para,bina=bina,stds=stds
...
dir='C:\Users\Samuel\Research\SSFR3\data\full\'
print, 'restoring the lut pdfs'
restore, dir+'pari_lut_pdf_v1.out'
restore, dir+'parl_lut_pdf_v1.out'
binl=binsl & bini=binsi
...
if pha then begin
  ...
  if 0 then begin
    para=fltarr(n_elements(pars[*,0,0,0]),n_elements(pars[0,*,0,0]),2,np,nb)
 ; paral=match_bins(parl,bminl,bmaxl,bmina,bmaxa)
    print, 'running the match bins'
    para[*,9:*,1,*,*]=match_bins(pari,bini,bmina,bmaxa,bins=bina)
    para[*,0:29,0,*,*]=match_bins(parl,binl,bmina,bmaxa)
    ref=findgen(60)+1.
    save, para,bina,tau,ref,filename=dir+'para_lut_pdf_v1.out'
    stop
  endif else restore, dir+'para_lut_pdf_v1.out' 
endif
end
...
; function to match the pars distribution based on bins of liq and ice to pars distribution based on all bins
function match_bins, pari,bin,bmina,bmaxa,bins=bins
;para[*,9:*,1,*,*]=match_bins(pari,bmini,bmaxi,bmina,bmaxa)
nt=n_elements(pari[*,0,0,0])
nr=n_elements(pari[0,*,0,0])
np=n_elements(pari[0,0,*,0])
nn=n_elements(pari[0,0,0,*])
bins=bin
par=pari
for p=0, np-1 do begin
  db=abs(bmaxa[p]-bmina[p])/nn
  bins[p,*]=findgen(1400)*db+bmina[p]
  for t=0, nt-1 do begin
    for r=0, nr-1 do begin
      pr=reform(pari[t,r,p,*])
      par[t,r,p,*]=interpol(pr,bin[p,*],bins[p,*])
      ;for n=0,nn-1 do begin
      ;  q=where(bin[p,*] ge bins[p,n] and bin[p,*] lt bins[p,n]+db,nq)
      ;  if nq gt 0 then par[t,r,p,n]=total(pr[q],/nan)
      ;endfor
      par[t,r,p,*]=par[t,r,p,*]/total(par[t,r,p,*],/nan)
   ;stop
    print, p,t,r
    endfor  ; ref loop
  endfor  ; tau loop
