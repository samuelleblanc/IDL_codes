...
is=intarr(nlbl,4)
  ...
  device, xsize=45, ysize=15
   ...
   !p.multi=[0,3,3,0,1] & !x.margin=[6,1] & !y.margin=[3,3] & !y.omargin=[1,0] & !x.omargin=[0,14]
...
for i=0, nlbl-1 do begin
  ...
  ii=ii-1
  for j=0,3 do taus[fl[0:ii[j],j],j]=smooth(taus[fl[0:ii[j],j],j],1)
  for j=0,3 do refs[fl[0:ii[j],j],j]=smooth(refs[fl[0:ii[j],j],j],1)
  ...
  if ii[0] gt 0 then begin
    oplot, utc[fl[0:ii[0],0]],smooth(taus[fl[0:ii[0],0],0],2),color=0,psym=4,thick=2
...
;    if ii[1] gt 0 then oplot, utc[fl[0:ii[1],1]],taus[fl[0:ii[1],1],1],color=50,psym=4,thick=2
    errplot,utc[fl[0:ii[0],0]],smooth(taus[fl[0:ii[0],0],0],2)-etaus[fl[0:ii[0],0],0],$
    ...
    errplot,utc[fl[0:ii[2],2]],smooth(taus[fl[0:ii[2],2],2],1)-etaus[fl[0:ii[2],2],2],$
      smooth(taus[fl[0:ii[2],2],2],1)+etaus[fl[0:ii[2],2],2] ,color=50,thick=2
    errplot,utc[fl[0:ii[3],3]],smooth(taus[fl[0:ii[3],3],3],1)-etaus[fl[0:ii[3],3],3],$
      smooth(taus[fl[0:ii[3],3],3],1)+etaus[fl[0:ii[3],3],3] ,color=250,thick=2
  ...
  plot, utc[0:n-1],utc[0:n-1]*0,/nodata,yrange=yrle[*,i],ytitle='r!De!N (!9m!Xm)',$
   xtickname=nul,ymargin=[-1.8,2.2],yticks=2,yticklen=0.05,xticklen=0.05,yminor=5
  if ii[0] gt 0 then begin 
    oplot, utc[fl[0:ii[0],0]],smooth(refs[fl[0:ii[0],0],0],2),color=0,psym=4,thick=2
    oplot, utc[fl[0:ii[2],2]],smooth(refs[fl[0:ii[2],2],2],1),color=50,psym=7,thick=2
    oplot, utc[fl[0:ii[3],3]],smooth(refs[fl[0:ii[3],3],3],1),color=250,psym=1,thick=2
    errplot,utc[fl[0:ii[0],0]],smooth(refs[fl[0:ii[0],0],0],2)-erefs[fl[0:ii[0],0],0],$
      smooth(refs[fl[0:ii[0],0],0],2)+erefs[fl[0:ii[0],0],0] ,color=0,thick=2
    errplot,utc[fl[0:ii[2],2]],smooth(refs[fl[0:ii[2],2],2],2)-erefs[fl[0:ii[2],2],2],$
      smooth(refs[fl[0:ii[2],2],2],2)+erefs[fl[0:ii[2],2],2] ,color=50,thick=2
    errplot,utc[fl[0:ii[3],3]],smooth(refs[fl[0:ii[3],3],3],2)-erefs[fl[0:ii[3],3],3],$
      smooth(refs[fl[0:ii[3],3],3],2)+erefs[fl[0:ii[3],3],3] ,color=250,thick=2
...
fp=dir+'..\plots\kisq\retr_samp_scat3'
  print, 'making plot :'+fp
  set_plot, 'ps'
  loadct, 39, /silent
  device, /encapsulated, /tt_font, set_font='Helvetica Bold',/color,bits_per_pixel=8., filename=fp+'.ps'
...
u=where(res[i,d] ge 10.)
rek[i[u],d]=rek[i[u],d]+1.
...
print, 'restoring ki'
restore, dir+'retrieved_kisq_'+lbl+'_v7.out'
