...
get_lut_sav,namelut,taus,refi,refl,pari,parl,bini,binl,np,refs,wps,pars, pexs,pha=pha, para=para, bina=bina,stds=0.01
...
if pha then begin
post_pdf=fltarr(nt,nr,nw,n_elements(taus),n_elements(refs),nw,np1)
post_meas_pdf=fltarr(nt,nr,nw,n_elements(taus),n_elements(refs),nw,np1)
post=fltarr(nt,nr,nw,n_elements(taus),n_elements(refs),nw)
  ...
  for t=0, nt-1 do begin
    ...
    for r=0,nr-1 do begin
      ...
      if pha then begin  ;run the non phase segregated method
        ...
        make_meas_pdf, sp,wvl,bina,meas_pdf,sno,ps=ps,dark_s=dark_s,resps=resps
