...
;  tvlct,160,230,160,4 & tvlct,160,230,190,3 & tvlct,160,230,220,2 & tvlct,160,230,250,1  
 ...
 if uu gt 0 then  oplot, wv, splow[1,17,*,0]/max(splow[1,17,*,0]),color=1
 if uu gt 1 then  oplot, wv, splow[2,17,*,0]/max(splow[2,17,*,0]),color=2
...
;  for i=0,3  do oplot, wv, splow[i,17,*,0]/max(splow[i,17,*,0]),color=i+1
  ...
  if uu gt 0 then for i=1,1  do oplot, wv, splow[i,17,*,1]/max(splow[i,17,*,1]),color=i
  if uu gt 1 then for i=2,2  do oplot, wv, splow[i,17,*,1]/max(splow[i,17,*,1]),color=i
  tvlct,0,0,0,0
  tvlct,170,222,255,10
  tvlct,0,0,107,238
  tvlct,255,175,90,160
...
print, 'making plot :'+fp
  ...
  for i=0,21<uu do oplot, wv, sp[i,17,*,0]/max(sp[i,17,181,0]),color=(i+5)*7
...
loadct,3
  revct
  for i=0,21<uu do oplot, wv, sp[i,17,*,1]/max(sp[i,17,181,1]),color=(i+1)*6
  tvlct,0,0,0,0
  i=i-1
