...
;
; PURPOSE:
;   Read in the data for calibrations, make response functions from primary calibration and secondary and compare multiple response functions
;
; CATEGORY:
;   ATTREX / Calibration
;
...
;   - all ".OSA2" files for the calibs under the proper directory (/attrex/cal/*date*/p3/(*lamp number*)/(nadir or zenith/(cal or dark))
;  
; EXAMPLE:
;   attrex_cal
;
...
; Modified: Tuesday, August 30th, 2011
;          -by Samuel LeBlanc
;           Change to ATTREX
;---------------------------------------------------------------------------
@cfg.pro
...
if(strcmp(dozenith,'yes')) then begin
  ...
  legend_tit=[legend_tit,'Primary Cal - '+primary_cal_date[i]+' - {'+strcompress(string(zsi_intime_p[i]),/REMOVE_ALL)+','+strcompress(string(zir_intime_p[i]),/REMOVE_ALL)+'}']
    ...
    legend,legend_tit[1:*],textcolors=[color_s,color_s],/right,outline_color=255,charsize=0.75 ;'
...
legend,['Primary : Secondary',leg_tit[1:*]],textcolors=[255,color_s],/right, outline_color=255,charsize=0.75
    p=tvrd(true=1)
