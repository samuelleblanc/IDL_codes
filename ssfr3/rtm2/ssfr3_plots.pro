...
set_plot, 'ps'
 ...
 !p.font=1 & !p.thick=4 & !p.charsize=2.0 & !x.style=1 & !y.style=1 & !z.style=1 & !y.thick=1.8 & !x.thick=1.8 & !p.multi=0 & !x.margin=[7,2]
...
cls=cls*250/max(cls)
 for i=1, n_elements(plot_wl)-1 do begin
  ...
  oplot, tmhrs_big[fl],nad_spect[fl,in],color=cls[i],thick=10
  lines=[lines,0]
 ...
 legend, replicate('|',n_elements(plot_wl)),color=[cls],box=0,linestyle=[lines],thick=[lines]+10, pspacing=1.6,$
