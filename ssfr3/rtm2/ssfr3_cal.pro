...
;
; PURPOSE:
;   Read in the data for calibrations of the irradiance light collector, make response functions from primary calibration 
;
; CATEGORY:
...
;           -by Samuel LeBlanc
;           transferred the calibration program from radiance to irradiance
;
;---------------------------------------------------------------------------
@ingest_ssfr.pro
@legend.pro
...
;lspectralon            = strsplit(cfg(cfg_file,'spectralon_reflectance'),', ',escape='#',/EXTRACT)
...
for i=0, num-1 do begin
 ...
 ; refl_spec[*,3] = interpol(sp.field1[1,*],sp.field1[0,*],wvl[*,3])
  ...
  ;do the Nadir InGaAs
  ;get the cal
  file=file_search(nadir_ir[i]+l+'cal'+l+data, count = numfiles, /FOLD_CASE)
  ...
  ;get the dark
  file=file_search(nadir_ir[i]+l+'dark'+l+data, count = numfiles, /FOLD_CASE)
  ...
  spec=cali-dark
  for ii=0, 3 do resp[*,ii]=spec[*,ii]/lamp_spec[*,ii] ;make the response functions 
...
if port eq 'zenith' then begin
  print, 'Irradiance head is connected to Zenith'
  ...
  print, 'Irradiance head is connected to Nadir'
  ...
  plot, wvl[*,2], resp_arr(*,2,ref),color=0,ytitle='%',xtitle='Wavelength (nm)',$
   ...
   thick=line_thick[ref],/xs,/ys,xrange=[300,2200],yrange=[-20,30], /nodata
  leg_tit='0'
  for i=0, num-1 do begin
