...
; is closed (this is called dark cycle), the dark spectra are measured, and averaged.
    ...
    darkmax=800                ; max of dark cycles, increase if needed
    ...
    for ind = 0, nf - 1 do begin ; go through all raw data files
     ...
     spect = {btime: lonarr(2), bcdtimstp:  bytarr(12), intime1: long(0), $
	    ...
	    if (mmml ge 45.0 and mmmp le 46.0) then sh=1
            ...
            if sh then begin
              if spect.zspecsi[45] gt 435 then sh=0
              if spect.nspecsi[45] gt 470 then sh=0            
              if spect.zspecir[45] gt -990 then sh=0
              if fix(date) gt fix('20120622') then if spect.nspecir[45] gt -795 then sh=0
              print, spect.zspecsi[45],'/435', spect.nspecsi[45],'/470', spect.zspecir[45],'/-9900', spect.nspecir[45],'/-795'
               ;if sh eq 0 then stop
            endif	
...
;save, zsi_hld, zir_hld, nsi_hld,  nir_hld, filename='/home/leblanc/SSFR3/dark_ex.out'
;stop
