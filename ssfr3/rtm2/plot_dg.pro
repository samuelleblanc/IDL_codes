...
for i=0, 19 do begin
  for j=0,19 do begin
    ...
    an[i,j]=(1.-g[i]^(2.))*(g[i]-1./mu[j])
    bn[i,j]=(1.+g[i]^2.-2./mu[j]*g[i])
    dp[i,j]=bn[i,j]^(3./2.)*(-3./2.*an[i,j]/bn[i,j]-g[i])^(-1.)
    for k=0,19 do begin
      for l=0, 20 do begin
      dg[i,j,k,l]=(mu[j]-1.)/(F_o*mu[j])*((exp((-1.)*mu[j]*tau[k])-exp((-1.)*tau[k]))*tau[k]*w[l])^(-1.)*dp[i,j]
      dw[i,j,k,l]=(mu[j]-1.)/(F_o*mu[j])*((exp((-1.)*mu[j]*tau[k])-exp((-1.)*tau[k]))*tau[k]*P[i,j])^(-1.)
...
nlvl=string(lvls[[0,4,9,14,19,24,29]],format='(G10.2)')

fn='/home/leblanc/DC3_SEAC4RS/SSFR3/plots/sensitivity_g'
     print, 'plotting:'+fn
     set_plot, 'ps'
     loadct, 39, /silent
...
ma=2.E6
     ...
     contour, reform(dg[10,*,*,19]),mu,tau,xtitle=ua, ytitle=ta, levels=lvls,/cell_fill,$
      title='Sensitivity of g to changing '+ua+' and '+ta;, xticks=4,/xlog;,min_value=mi,max_value=ma

     contour, reform(dg[*,*,10,19]),g,mu,xtitle='g', ytitle=ua, levels=lvls,/cell_fill,$
      title='Sensitivity of g to changing g and '+ua;, yticks=4,/ylog;,min_value=mi,max_value=ma

     contour, reform(dg[*,10,*,19]),g,tau,xtitle='g', ytitle=ta, levels=lvls,/cell_fill,$
      title='Sensitivity of g to changing g and '+ta;, min_value=mi,max_value=ma

     contour, reform(dg[10,10,*,*]),tau,w,xtitle=ta, ytitle=wa, levels=lvls,/cell_fill,$
      title='Sensitivity of g to changing '+ta+' and '+wa;, min_value=mi,max_value=ma
...
;     colorbar, minrange=mi,maxrange=ma, title=de+'g/'+de+'I',/vertical,/right,format='(G10.2)', position=[0.87,0.13,0.89,0.93]
   ...
   contour, transpose([[findgen(30)],[findgen(30)]]),[0,1],findgen(30),/cell_fill,nlevels=30,position=[0.87,0.13,0.89,0.93],/normal,/noerase,xticks=1,yticks=1,xtickname=[' ',' '],ytickname=[' ',' ']  ;take this out to omit residual
    axis, yaxis=1,ystyle=1,yrange=[min(lvls),max(lvls)],ytitle=de+'g/'+de+'I',yticks=6,ytickname=nlvl


    device, /close
    spawn, 'convert '+fn+'.ps '+fn+'.png'
    
    fn='/home/leblanc/DC3_SEAC4RS/SSFR3/plots/sensitivity_ssa'
     print, 'plotting:'+fn
     set_plot, 'ps'
     loadct, 39, /silent
     device, /encapsulated, /tt_font, set_font='Helvetica Bold'
...
ma=0
     contour, reform(dw[10,*,*,19]),mu,tau,xtitle=ua, ytitle=ta, levels=lvls,/cell_fill,$
      title='Sensitivity of '+wa+' to changing '+ua+' and '+ta;, xticks=4,/xlog;,min_value=mi,max_value=ma

     contour, reform(dw[*,*,10,19]),g,mu,xtitle='g', ytitle=ua, levels=lvls,/cell_fill,$
      title='Sensitivity of '+wa+' to changing g and '+ua;, yticks=4,/ylog;,min_value=mi,max_value=ma

     contour, reform(dw[*,10,*,19]),g,tau,xtitle='g', ytitle=ta, levels=lvls,/cell_fill,$
      title='Sensitivity of '+wa+' to changing g and '+ta;, min_value=mi,max_value=ma

     contour, reform(dw[10,10,*,*]),tau,w,xtitle=ta, ytitle=wa, levels=lvls,/cell_fill,$
      title='Sensitivity of '+wa+' to changing '+ta+' and '+wa;, min_value=mi,max_value=ma

    ; colorbar, minrange=mi,maxrange=ma, title=de+wa+'/'+de+'I',/vertical,/right,format='(G10.2)', position=[0.87,0.13,0.89,0.93]

contour, transpose([[findgen(30)],[findgen(30)]]),[0,1],findgen(30),/cell_fill,nlevels=30,position=[0.87,0.13,0.89,0.93],/normal,/noerase,xticks=1,yticks=1,xtickname=[' ',' '],ytickname=[' ',' ']  ;take this out to omit residual
    axis, yaxis=1,ystyle=1,yrange=[min(lvls),max(lvls)],ytitle=de+wa+'/'+de+'I',yticks=6,ytickname=nlvl
