...
pro arctas_rtm_aero, datein, second=second, save=save, plotting=plotting, video=video, divergence=divergence,$
 ...
 choose=choose, angstrom=angstrom, error_loop=error_loop, hsrl=hsrl, unique=unique
...
if unique then begin
  if unique_ct ge n_unique then unique_loop=0 else begin
    tau_scale=float(unique_ct)*0.05
...
if not ( keyword_set(choose) or keyword_set(find_legs)) then begin
;ind=findgen(30)
;print, 'modifiying ind to 30'
;ind=10
ind=findgen(n_elements(fl))
endif
...
fl_f=30; n_elements(fl)-1 ;number of points to go through
...
for iw=0, iw_f do begin ;wavelength loop
  ...
  for i=fl_s, fl_f do begin ;lon loop
    ...
    while tau_loop do begin
      ...
      print, 'tau: ',tau_mod*tau_init, 'actual tau:', tau[index,aero_wl]*tau_mod*tau_init
