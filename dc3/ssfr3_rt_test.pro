...
;
; PURPOSE:
;   A test using near real-time data processing for SSFR3, processes OSA data and then saves
;   it to an idl out file, for further plotting and cloud properties retrieval.
;   Supports incoming files of OSA files
;
; CATEGORY:
;   SSFR3 roof top real-time processing testing
;
; CALLING SEQUENCE:
;   ssfr3_rt_test,date,dir
;   - where dir is the directory within the specified path to watch for incoming files
;   - date is the string of the date required in yyyymmdd format
;
; OUTPUT:
...
;  
; EXAMPLE:
;  ssfr3_rt, '20120605','/home/leblanc/DC3_SEAC4RS/SSFR3/'
;
;
...
; Modified: June 6th, 2012, LASP CU Boulder
;           - by Samuel LeBlanc
;           - changed the rt to a test program
;           - added the date variable
;---------------------------------------------------------------------------
...
outdir=path+'outtest'+l ;path to save the idl out files      ; change this at dryden
...
while not strcmp(action,'s') do begin
  ...
  if new_ssfr then begin ; enter the plotting and saving routine
   ...
   ; if doplot then ssfr3_rt_plots, wvl, utc, spect, spect_cal, sza, extra, rnav, ssfr_temp
    if ctr_sav eq 0 then f=file_search(outdir+date+'_RT_*_drk.out',count=nout) else nout=0
    ov=''
    ...
    save, utc,lat,lon,sza,dc,facz,dz,facn,dn,n_sat,$
     spect,spect_cal,zspectra,nspectra,ssfr_temp,attcorr,iza,filename=outdir+date+'_RT_'+string(ctr_sav,format='(I03)')+'_drk.out'
