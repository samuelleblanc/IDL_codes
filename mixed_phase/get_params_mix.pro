...
; Knap parameter used in cloud phase detection by AVIRIS (Knap et al. 2002)
nul=min(abs(wvl-1640.),io)
nul=min(abs(wvl-1700.),in)
par[15]=(sp[in]-sp[io])/sp[io]*100.

; for seventeen parameter
; use the Is definition by Ehrlich et al. 2008
; it simply extends the Knap definition to use 1550 nm instead of 1640 nm
nul=min(abs(wvl-1550.),io)
nul=min(abs(wvl-1700.),in)
if sp[io] eq 0. then sp[io]=2.E-12
par[16]=(sp[in]-sp[io])/sp[io]*100.
