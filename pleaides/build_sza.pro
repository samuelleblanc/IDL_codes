...
indir ='/lustre/janus_scratch/leblanse/cloud/input/szas2/'
outdir='/lustre/janus_scratch/leblanse/cloud/output/szas2/'
dir   ='/projects/leblanse/cloud/'

spawn,'mkdir '+indir
spawn,'mkdir '+outdir
;spawn,'mkdir '+dir
list_file='/projects/leblanse/cloud/run_szas2.sh'
...
time=findgen(12)*0.25+19.20
doy=julian_day(strmid(date,0,4),strmid(date,4,2),strmid(6,2))
zensun, doy,time,lat, lon, sza, azi, solfac
sza=findgen(20)+17.0
...
zout=[0.02]
  for t=0, n_elements(tau)-1 do begin
    for r=0, n_elements(ref)-1 do begin
      for s=0, n_elements(sza)-1 do begin
          ...
          wvl=[500.,504.]
          albedo=0.2
   write_input_file, doy, sza[s], atm_file,atm_file,inf,0.0,0.9,0.9,wvl,zout,albedo,/radiance,/quiet,cloud=cloud,alb_file=alb_file
          ...
          outf=outdir+fn+'.out'
          inf =indir +fn+'.in'
          write_input_file, doy, sza[s], atm_file,atm_file,inf,0.0,0.9,0.9,wvl,zout,albedo,$
...
if n_elements(new) gt 0 then printf,ui,'deltam on'
printf,ui,'quiet'
free_lun,ui
