...
indir ='/lustre/janus_scratch/leblanse/cloud/input/phase/'+name+'/';'/scratch/stmp00/leblanse/cloud/input/'
outdir='/lustre/janus_scratch/leblanse/cloud/output/phase/'+name+'/';'/scratch/stmp00/leblanse/cloud/output/'
dir   ='/projects/leblanse/cloud/'
;phase =dir+'phase/v2/hollow_bullet_rosettes050.leg' ;large_aggregate_plates050.leg' ;hollow_bullet_rosettes050.leg' ;aggregate_columns050.leg' ;large_aggregate_plates050.leg' 
; solid_bullet_rosettes050_small.leg' ;hollow_bullet_rosettes050_small.leg' ;large_aggregate_plates050.leg'
;phase=dir+'phase/GeneralHabitMixture_Smooth.leg' ;GeneralHabitMixture_ModeratelyRough.leg' ;GeneralHabitMixture_SeverelyRough.leg'
;phase=dir+'phase/v2/small_aggregate_plates050.leg'
phase=dir+'phase/v2/aggregate_columns050.leg'
;hollow_bullet_rosettes000.leg' ;hollow_columns050.leg' ;solid_bullet_rosettes050.leg' ;solid_columns050.leg'
restore, phase
stop
spawn,'mkdir '+indir
spawn,'mkdir '+outdir
;spawn,'mkdir '+dir
list_file='/projects/leblanse/cloud/run_phase_'+name+'.sh'
...
alb_file='/lustre/janus_scratch/leblanse/sea_albedo.dat' ;'/lustre/janus_scratch/leblanse/albedo_eMAS.dat'
...
zout=[0.02,5.]
  for t=0, n_elements(tau)-1 do begin
    for r=0, n_elements(ref)-1 do begin
          ...
          fn='cloud_'+name+'_ta'+string(t,format='(I02)')+'_re'+string(r,format='(I03)')          
          outf=outdir+fn+'.out'
          inf =indir +fn+'.in'
...
;          endif 
          ...
          inf =indir +fn+'.in'
          write_input_file, doy, sza, atm_file,atm_file,inf,0.0,0.9,0.9,wvl,zout,albedo,$
           /radiance,/quiet,cloud=cloud,alb_file=alb_file,/ice,/ic_files,phase=phase
          printf, uu, '/projects/leblanse/libradtran/libRadtran-1.6-beta/bin/uvspec < '+inf+' > '+outf
          print, inf
    endfor   ;ref loop
  endfor    ;tau loop
...
if n_elements(cloud) gt 0 then begin
  ...
  if n_elements(ic_files) eq 0 then begin
    ...
    printf,uc,cloud[2]-0.5*cloud[3],' '+input_file+'cld0.dat',format='(F12.6," ",A)'
    make_ic_files, input_file+'cld0.dat', [0,cloud[1],cloud[2],cloud[3]],phase=phase
    make_ic_files, input_file+'cld1.dat', cloud,phase=phase
  endelse
  free_lun,uc
endif
...
printf,ui,'solar_file        /projects/leblanse/libradtran/solar_eMAS.dat' ;/projects/leblanse/libradtran/libRadtran-1.6-beta/data/solar_flux/kurudz_0.1nm.dat';/projects/leblanse/libradtran/solar_SSFR.dat'
...
if keyword_set(radiance) then begin
  ...
  printf,ui,'umu               -1  1   # -1:downward radiance; 1:upward radiance; 0:sidward radiance' ;for radiances (ship)
...
if n_elements(ice) gt 0 then printf,ui,'deltam on'
printf,ui,'quiet'
free_lun,ui
