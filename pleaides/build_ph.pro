...
indir ='/lustre/janus_scratch/leblanse/cloud/input/zna/'
outdir='/lustre/janus_scratch/leblanse/cloud/output/zna/'
dir   ='/projects/leblanse/cloud/'

spawn,'mkdir '+indir
spawn,'mkdir '+outdir
;spawn,'mkdir '+dir
list_file='/projects/leblanse/cloud/run_zna.sh'
    ...
    for r=0, n_elements(ref)-1 do begin
      for s=0, n_elements(zna)-1 do begin
          ...
          write_input_file, doy, sza, atm_file,atm_file,inf,0.0,0.9,0.9,wvl,zout,albedo,$
           /radiance,/quiet,cloud=cloud,alb_file=alb_file,zna=zna[s]
          ...
          inf =indir +fn+'.in'
          write_input_file, doy, sza, atm_file,atm_file,inf,0.0,0.9,0.9,wvl,zout,albedo,$
           /radiance,/quiet,cloud=cloud,alb_file=alb_file,/ice,/ic_files,zna=zna[s]
          printf, uu, '/projects/leblanse/libradtran/libRadtran-1.6-beta/bin/uvspec < '+inf+' > '+outf
          print, inf
       endfor  ;sza loop
    endfor   ;ref loop
...
if keyword_set(radiance) then begin
  ...
  if n_elements(zna) gt 0 then printf,ui,'umu               '+string(cos((zna-180.)*!dtor)) else $
