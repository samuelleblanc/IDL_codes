    ...
    for r=0, n_elements(ref)-1 do begin
       cloud=[tau[t],ref[r],0.3+1.68,1.5]
        if ref[r] le 30. then begin       
          ...
          outf=outdir+fn+'.out' & inf =indir +fn+'.in' & inf2=indir+'../v1_20120912/'+fn+'.in'
          wvl=[400.,981.]
          write_inp_mix, doy, sza, atm_file, inf, azi, wvl, zout, /nofile,noff=inf2,$ 
           /quiet, cloudl=cloud,alb_file=alb_file,cloudi=cloud, wp=0.,wc_mie=wc_mie,/kurudz,/slit
        ...
        if ref[r] ge 5. then begin
          ...
          wvl=[400.,981.]
          write_inp_mix, doy, sza, atm_file, inf, azi, wvl, zout,/nofile,noff=inf2,$
           /quiet, cloudl=cloud,alb_file=alb_file,cloudi=cloud, wp=1.,ic_dat=ic_dat,/kurudz,/slit
          printf, uu, '/projects/leblanse/libradtran/libRadtran-1.7/bin/uvspec < '+inf+' > '+outf
