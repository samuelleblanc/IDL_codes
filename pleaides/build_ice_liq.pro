...
indir ='/lustre/janus_scratch/leblanse/cloud/input/ice_liq_new/';'/scratch/stmp00/leblanse/cloud/input/'
outdir='/lustre/janus_scratch/leblanse/cloud/output/ice_liq_new/';'/scratch/stmp00/leblanse/cloud/output/'
dir   ='/projects/leblanse/cloud/'

spawn,'mkdir '+indir
spawn,'mkdir '+outdir
;spawn,'mkdir '+dir
list_file='/projects/leblanse/cloud/run_cloud_ice_liq_new.sh'
...
r=0
  for t=0, n_elements(tau)-1 do begin
          if t lt 10 then fn='cloud_ice_liq_'+string(tau[t],format='(I02)') else fn='cloud_ice_liq_xx'          
          outf=outdir+fn+'.out'
          inf =indir +fn+'.in'
          cloud=[tau[t],ref[r],1.5+1.68,1.5]
          wvl=[400.,2200.]
          albedo=0.2
;   write_input_file, doy, sza, atm_file,atm_file,inf,0.0,0.9,0.9,wvl,zout,albedo,/radiance,/quiet,cloud=cloud,alb_file=alb_file
;          printf, uu, '/projects/leblanse/libradtran/libRadtran-1.6-beta/bin/uvspec < '+inf+' > '+outf
          ...
          write_input_file, doy, sza, atm_file,atm_file,inf,0.0,0.9,0.9,wvl,zout,albedo,$
           /radiance,/quiet,cloud=cloud,alb_file=alb_file,/ice,/ic_files,/wc_files
          printf, uu, '/projects/leblanse/libradtran/libRadtran-1.6-beta/bin/uvspec < '+inf+' > '+outf
          print, inf
;    endfor   ;ref loop
  endfor    ;tau loop
...
pro write_input_file, doy, sza, tau_file, atm_file, input_file, azimuth, asy, ssa, wvl, zout, albedo, radiance=radiance,$
  quiet=quiet, tau_scale=tau_scale, slit=slit, nofile=nofile, aod=aod,cloud=cloud,h2o=h2o,alb_file=alb_file,ice=ice,ic_files=ic_files,wc_files=wc_files
...
if n_elements(cloud) gt 0 then begin
  ...
  ref=cloud[1]
  uc=98
  if n_elements(wc_files) gt 0 then cloud[2]=cloud[2]+1.5 ;make ice cloud higher than liquid cloud 
  ...
  if n_elements(ic_files) eq 0 then begin
    ...
    printf,uc,cloud[2]+0.5*cloud[3],' NULL',format='(F12.6," ",A)'
    printf,uc,cloud[2]  ,' '+input_file+'cld1.dat',format='(F12.6," ",A)'
    make_ic_files, input_file+'cld1.dat', cloud
  endelse
  free_lun,uc

  if n_elements(wc_files) gt 0 then begin
  openw ,uc,input_file+'wccld.dat'
    printf,uc,cloud[2]-1.5+0.5*cloud[3],' NULL',format='(F12.6," ",A)'
    printf,uc,cloud[2]-1.5  ,' '+input_file+'wccld1.dat',format='(F12.6," ",A)'
    make_wc_files, input_file+'wccld1.dat', [100.-cloud[0],cloud[1],cloud[2],cloud[3]]
  free_lun,uc
  endif
endif
...
if n_elements(cloud) lt 1 then begin
  ...
  if n_elements(ice) gt 0 then begin
    ...
    if n_elements(wc_files) gt 0 then begin
      printf,ui,'wc_files '+input_file+'wccld.dat'
      printf,ui,'wc_layer'
    endif
  endif else begin  
