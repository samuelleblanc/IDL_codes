...
if(read_nav) then begin
    read_nav_knorr,navlocation,utcnav,latnav,lonnav
    lat_all=interpol(latnav,utcnav,tmhrs_all) ; interpolate lat/lon to SSFR-7 times
    lon_all=interpol(lonnav,utcnav,tmhrs_all)
    lat = lat_all
    lon = lon_all
endif else begin
    numobs = n_elements(tmhrs)
    lat = fltarr(numobs)
    lon = fltarr(numobs)
...
if(do_retrieval) then begin
    ...
    retrieve_cloud_parms,lutdir,inputdate,nspectra,nadlambda,tmhrs,lat,lon,tau,reff,tauunc,reffunc
endif
...
save,tmhrs,ql_zenlambda,zpartspec,ql_nadlambda,npartspec,cg4,lat,lon,status,filename=outfile
spawn, 'chmod a+w '+outfile
spawn,'gzip -9f '+outfile
...
time_indices = indgen(1+ctr/numsec, /double)*numsec
...
for ind1=0,ntime_indices-2 do begin
    ;;for ii=0,nqlwl-1 do begin
