...
pro calnex_aerosol_retrieval, datein , second=second, save=save, plotting=plotting, video=video, divergence=divergence,$
 ...
 choose=choose, angstrom=angstrom, aats=aats, interactive=interactive, interactive_ssa=interactive_ssa
...
if plotting then begin
  ...
  ;plotting tau
  axis, yaxis=1, /device, yrange=[0,1.], /save, color=200, ytitle='HSRL tau'
...
for iw=0, iw_f do begin ;wavelength loop
  ...
  lunn=98  
  openw, lunn, dirout+file_n+date+'_wvl'+strtrim(string(fix(wvl_aod[aero_wl]),format='(I4.4)'),2)+secon+'.txt';,/get_lun
  ...
  for i=fl_s, fl_f do begin ;track loop
   ...
   if aeronet then  mm=min(abs(jul_day-doy+utc[legs.leg1[index]]/24.0),aeronet_index)  ;get the closest in time aeronet index
    ...
    while tau_loop do begin
      ...
      if aats then tau_now=tau[index,aero_wl]*tau_mod*tau_init else tau_now=tau[index]*tauscale*tau_init
    ...
    if (abs(asy-asy2) lt 0.025) and not keyword_set(sensitivity) and count_ssa lt 51 and count_asy lt 41 and count_asy2 lt 41 then begin
