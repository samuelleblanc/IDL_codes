...
calnex_crd, dateinp, time, ext, ext_ambient, abs
...
endcase
  ...
  if leg1[n_elements(leg1)-1] gt leg2[0] then ind_heights=[leg2[n_elements(leg2)-1],leg1[0]] else ind_heights=[leg1[n_elements(leg1)-1],leg2[0]]    ; from last point on bottom leg to first on top leg
  ...
  extinction=fltarr(n_heights,findgen(n_elements(leg1)))
  ...
  nbelow=nspectra[*,leg1]*0.0
    ...
    for i=0, n_elements(leg1)-1 do begin
      ...
      for j=0,n_heights do begin
	    extinction[j,i]=ext_profile[j]*((ext_ambient[leg1[i]]/ext_ambient[ind_heights[0]])*(1-j/n_heights)+(ext_ambient[near_leg2[i]]/ext_ambient[ind_heights[0]])*(j/n_heights)
      endfor
...
contour,t_e,lon[leg1],z,nlevels=30,/cell_fill, title='Extinction coefficents profiles', $
 xtitle='Longitude (degrees)', ytitle='Altitude (m)', yrange=[0,3000], xrange=[-118.21,-118.01], position=[0.12,0.18,0.45,0.9], xticks=4,min_value=min(transpose(e),/nan), max_value=0.45;max(transpose(e),/nan)  ;,xrange=[-118.13,-118.01]
...
plot, lon[leg1],lat[leg1], title='', $
 ...
 yticks=2, /noerase
  ...
  for i=1,59 do begin
    c_n=fix(255/(0.45-min(transpose(e),/nan)) * (t_e[i]-min(transpose(e),/nan)))
    oplot, [lon[leg1[i]],lon[leg1[i]]],[lat[leg1[i]],lat[i]]],color=c_n, psym=8, symsize=1.8
  endfor
endif
xyouts, lon_cal, lat_cal-0.01, 'Caltech', charsize=1.5
oplot, [lon_cal,lon_cal],[ lat_cal,lat_cal], psym=2
...
stop
save, legs,sza,z,alt,extinction,aot,utc, lat, lon,nspectra,zspectra,nadlambda,zenlambda,near, filename=calnex_dir+pathout
