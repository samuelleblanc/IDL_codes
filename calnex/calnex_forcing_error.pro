...
for iw=0, iw_f do begin
  ...
  for_err[iw,0]=sqrt(((forcing[iw,2,0]-forcing[iw,1,0])/err[0,iw])^2+((forcing[iw,4,0]-forcing[iw,3,0])/err[1,iw])^2+((forcing[iw,6,0]-forcing[iw,5,0])/err[3,iw])^2+((forcing[iw,8,0]-forcing[iw,7,0])/err[4,iw])^2)
  for_err[iw,1]=sqrt(((forcing[iw,2,1]-forcing[iw,1,1])/err[0,iw])^2+((forcing[iw,4,1]-forcing[iw,3,1])/err[1,iw])^2+((forcing[iw,6,1]-forcing[iw,5,1])/err[3,iw])^2+((forcing[iw,8,1]-forcing[iw,7,1])/err[4,iw])^2)
  eff_err[iw,0]=sqrt((forcing[iw,0,0]*err[0,iw]/(aot[3,0]^2*rad_dn[iw,0,1]))^2+(forcing[iw,0,0]*rad_dn[iw,0,1]*0.05/(aot[3,0]*rad_dn[iw,0,1]^2))^2+(for_err[iw,0]/(aot[3,0]*rad_dn[iw,0,1]))^2)
  eff_err[iw,1]=sqrt((forcing[iw,0,1]*err[0,iw]/(aot[3,0]^2*rad_dn[iw,0,1]))^2+(forcing[iw,0,1]*rad_dn[iw,0,1]*0.05/(aot[3,0]*rad_dn[iw,0,1]^2))^2+(for_err[iw,1]/(aot[3,0]*rad_dn[iw,0,1]))^2)
endfor
...
save, for_err, eff_err, filename='/home/leblanc/CALNEX/forcing_err_calnex.out'
stop
