...
; Written:  Samuel LeBlanc, LASP CU Boulder, August 16th, 2010
; Modified: by Samuel LeBlanc, June 4th, 2012
;           - changed the program from calnex to attrex data 
;---------------------------------------------------------------------------

pro attrex_atmosphere
; create atmosphere and cloud from sounding, standard atmosphere,
; and 3D LES cloud
if !VERSION.OS eq 'linux' then linux=1 else linux=0 
date='20111105'
if linux then begin
  standard_atmosphere='/home/leblanc/libradtran/libRadtran-1.5-beta/data/atmmod/afglt.dat'
...
;  sounding='/home/leblanc/libradtran/input/aero/sounding_'+date+'.dat'
  ...
  gh_metfile='/data/seven/schmidt/attrex/gh/'+date+'/'+date+'_met.out'
endif else begin
  standard_atmosphere='\\lasp-smb\leblanc\libradtran\libRadtran-1.5-beta\data\atmmod\afglt.dat''
...
; flight met data from the Global Hawk

;calnex_met, date=date, outfile=outfile
;met=read_ascii(outfile, data_start=2)
restore, gh_metfile
fl=where(utc gt 24.0 and utc lt 27.0)
Z_met=alt[fl]
P_met=pre[fl]
T_met=tem[fl]
;H_met=met.field1[3,*]
...
;z_met=[0]
h_met=h_atm ;since no humidity measurement on GH
...
!p.multi=[0,2,2]
range=[0.0,10.0]
plot,h,t,title='Temperature',xtit='Height [km]',ytit='T [K]';,xrange=range,yrange=[260,295]
oplot,h,t,color=130,psym=2
oplot,h_met,T_met, color=250, psym=2

oplot,Z_met,T_met, color=250, psym=2
plot,h,p,title='Pressure',xtit='Height [km]',ytit='Pressure [hPa]';,xrange=range, yrange=[500,1050]
oplot,h,p,psym=2,color=130

oplot,Z_met,P_met, color=250, psym=2
plot,h,h2o,title='Water Amount',xtit='Height [km]',ytit='h2o [#/cm3]';,xrange=range,yrange=[0.1E16, 5E17]
