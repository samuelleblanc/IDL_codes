...
;   attrex_dark.pro  ;dark spectra finder (procedure ini_dark and read_drk)
;   
; NEEDED FILES:
;   - config file for day
;   - all ".OSA2" files for the day under the proper directory (/attrex/p3/*date*/)
;  
; EXAMPLE:
;   attrex_data, 20100421
;
...
indir  = '/data/seven/schmidt/attrex/gh'           ; directory
...
cfg_file=dir+l+'gh.cfg'        ; build cfg file
...
bignum  = 90000L ; defines maximum spectrum number - increase if needed
...
sat     = intarr(bignum)
sraw    = fltarr(4,bignum,np)   ; array with all the raw spectra ;0-zsi  1-zir 2-nsi 3-nir
scal    = fltarr(4,bignum,np)   ; array with all the calibrated spectra
sdrk    = fltarr(4,bignum,np)   ; array with all the dark spectra
    ...
    while not eof(lun) do begin ; Read individual spectra
        ...
        if (spect.shsw eq 0) then begin ; shutter open
           ; Get analogue channels and convert to voltage / temperature
            ...
            znirdrkr=fltarr(np)      & nnirdrkr=fltarr(np)
            for k=0,np-1 do begin
                ...
                znirdrkr[k] = znirdark[255-k]
                nnirdrkr[k] = nnirdark[255-k]
            endfor
            ...
            ; save the spectras
            sraw[0,ctr,*]=rawzsispectrum & sraw[1,ctr,*]=rawzirreverse & sraw[2,ctr,*]=rawnsispectrum & sraw[3,ctr,*]=rawnirreverse 
            scal[0,ctr,*]=zsispectrum    & scal[1,ctr,*]=zirspectrum   & scal[2,ctr,*]=nsispectrum    & scal[3,ctr,*]=nirspectrum  
            sdrk[0,ctr,*]=zvisdark       & sdrk[1,ctr,*]=znirdrkr      & sdrk[2,ctr,*]=nvisdark       & sdrk[3,ctr,*]=nnirdrkr
...
outfile = strcompress(outdir+l+strcompress(date,/remove_all)+'_temp.out',/REMOVE_ALL)
save, tmhrs, nsitemp, zsitemp, nirtemp, zirtemp, bxtemp, bxcltemp, nirxt, zirxt,filename = outfile
spawn, 'chmod a+w '+outfile

if date eq '20111020' or date eq '20111022' or date eq '20111024' then begin
set_plot,'x'
window, 0, title='calibrated spectra'
!p.multi=[0,1,2] & !p.thick=1.5
loadct, 39,/silent
device,decomposed=0
!p.background=255
!p.color=0
plot, tmhrs,scal[0,*,100],title='Zenith time series for Calibrated', xtitle='UTC (hours)',ytitle='Irradiance (W/m!U2!N nm)'
oplot, tmhrs,scal[1,*,150], color=250

legend,[strtrim(wlvisz[100],2),strtrim(wlnirz[150],2)]+' nm',textcolors=[0,250],box=0

plot, tmhrs,scal[2,*,100],title='Nadir time series', xtitle='UTC (hours)',ytitle='Irradiance (W/m!U2!N nm)'
oplot, tmhrs,scal[3,*,150], color=250
legend,[strtrim(wlvisn[100],2),strtrim(wlnirn[150],2)]+' nm',textcolors=[0,250],box=0
                        filestr = dir + l + 'Cal_spect.png'
                        rgb=tvrd(true=1)
                        write_png,filestr,rgb
                        
window, 1, title='raw spectra'
plot, tmhrs,sraw[0,*,100],title='Zenith time series for Raw', xtitle='UTC (hours)',ytitle='DN'
oplot, tmhrs,sraw[1,*,150], color=250

legend,[strtrim(wlvisz[100],2),strtrim(wlnirz[150],2)]+' nm',textcolors=[0,250],box=0

plot, tmhrs,sraw[2,*,100],title='Nadir time series', xtitle='UTC (hours)',ytitle='DN'
oplot, tmhrs,sraw[3,*,150], color=250
legend,[strtrim(wlvisn[100],2),strtrim(wlnirn[150],2)]+' nm',textcolors=[0,250],box=0
                        filestr = dir + l + 'Raw_spect.png'
                        rgb=tvrd(true=1)
                        write_png,filestr,rgb
                        
window, 2, title='Dark spectra'
plot, tmhrs,sdrk[0,*,100],title='Zenith time series for Darks', xtitle='UTC (hours)',ytitle='DN', yrange=[0,17000]
oplot, tmhrs,sdrk[1,*,150], color=250
oplot, tmhrs,sdrk[1,*,50], color=70
oplot, tmhrs,sdrk[1,*,200], color=170

legend,[strtrim(wlvisz[100],2),strtrim(wlnirz[150],2),strtrim(wlnirz[50],2),strtrim(wlnirz[200],2)]+' nm',textcolors=[0,250,70,170],box=0

plot, tmhrs,sdrk[2,*,100],title='Nadir time series', xtitle='UTC (hours)',ytitle='DN', yrange=[-200,3500]
oplot, tmhrs,sdrk[3,*,150], color=250
oplot, tmhrs,sdrk[3,*,50], color=70
oplot, tmhrs,sdrk[3,*,200], color=170
legend,[strtrim(wlvisn[100],2),strtrim(wlnirn[150],2),strtrim(wlnirn[50],2),strtrim(wlnirn[200],2)]+' nm',textcolors=[0,250,70,170],box=0
                        filestr = dir + l + 'Dark_spect.png'
                        rgb=tvrd(true=1)
                        write_png,filestr,rgb
endif
