...
uu0=0
if n_elements(strsplit(i0,' ,',escape='#',/EXTRACT)) gt 1 then begin
...
bb2=float(bb2)
if strcmp(platform,'GH',3,/FOLD_CASE) then noaa=1 else noaa=0
...
pitchoff = float(cfg(cfg_file,'pitchoff'))
rolloff  = float(cfg(cfg_file,'rolloff'))
; get platform
if strcmp(strmid(nav,0,1),'n',/FOLD_CASE) then nav=0 else nav=1
if nav then begin
 if strcmp(platform,'GH',3,/FOLD_CASE) then begin
   nav_f=file_search(dir+ll+date+ll+'IWG1_'+date+'_all.nav')	 
   daten=date
   ingest_nav, nav_f[0],daten,pitchoff,rolloff,utc_nav,lat,lon,alt,sza,iza,dc,rol,pit,hed,pre,t_ambient,0.0
   ;now interpol to ssfr times
   iza=interpol(iza,utc_nav,utc) & sza=interpol(sza,utc_nav,utc) & dc=interpol(dc,utc_nav,utc)   & lat=interpol(lat,utc_nav,utc)
   lon=interpol(lon,utc_nav,utc) & alt=interpol(alt,utc_nav,utc) & rol=interpol(rol,utc_nav,utc) & pit=interpol(pit,utc_nav,utc)
   hed=interpol(hed,utc_nav,utc) & pre=interpol(pre,utc_nav,utc) & t_ambient=interpol(t_ambient,utc_nav,utc) 
...
if strcmp(platform,'GH',3,/FOLD_CASE) then begin
  if iscg4 then begin
   cg4_f=file_search(dir+ll+date+ll+'*.CG4',count=ncg4)
   for i=0,ncg4-1 do begin
     infocg4=file_info(cg4_f[i])
     if infocg4.size ne 7600 then begin
       print, 'CG4 file:'+cg4_f[i]+' is corrupted, ignoring'
       continue
     endif   
     ingest_cg4, cg4_f[i], cfg_file, utc_cg4tm, cg4dntm, cg4uptm, zenttm, nadttm
     if n_elements(utc_cg4) eq 0 then begin
       utc_cg4=utc_cg4tm & cg4dns=cg4dntm & cg4ups=cg4uptm & zents=zenttm & nadts=nadttm
     endif else begin
       utc_cg4=[utc_cg4,utc_cg4tm] & cg4dns=[cg4dns,cg4dntm] & cg4ups=[cg4ups,cg4uptm] & zents=[zents,zenttm] & nadts=[nadts,nadttm]
     endelse
   endfor
   ;set the 24+ utc
   pp=where(utc_cg4 - shift(utc_cg4,1) lt 0, npl)
   if npl gt 0 then begin
     pp= pp[where(pp gt 2)]
     utc_cg4[pp[1]:*]=utc_cg4[pp[1]:*]+24.0
   endif
  ;stop 
   ; now interpol to the ssfr utc times
   flc=where(cg4dns ne -9999.0 and finite(cg4dns) eq 1,n)
   ;stop
   if n gt 0 then begin
     cg4dn=interpol(cg4dns[flc],utc_cg4[flc],utc) & cg4up=interpol(cg4ups[flc],utc_cg4[flc],utc) & zent=interpol(zents[flc],utc_cg4[flc],utc) & nadt=interpol(nadts[flc],utc_cg4[flc],utc)
   endif else begin
     message, 'All CG4 data is erronous'
   endelse
...
if plot and iscg4 then begin
  ...
  fc=where(zent gt -200 and nadt gt -200 and zent lt 70 and nadt lt 70 and cg4dn gt -9000 and cg4up gt -9000)
  ...
  linestyle = 0,xr=uu,/xs, yrange=[-100.0,70.0]
  ...
  linestyle =1,xr=uu,/xs,yrange=[-50.,700.],color=255
...
if plot then begin
  if contours then begin           ; only if contour is running
    ...
    device,/close
    spawn,  'convert "'+ dir+date+ll+'SF'+date+'_contour_'+platform+'.ps" "'+dir+date+ll+'SF'+date+'_contour_'+platform+'.png"'
...
;oplot, utc[flt],fac[flt], color=10, psym=3
  ...
  legend,outtxt,textcolors=outclr, charsize=1.5

  print,'DN=',dn[nind]
  plot,utf,nad[0,*]*dn[nind[0]]/(zen[0,*]*fac[flt,nind[0]]),xr=uu,yr=[0.,1.1],xtitle='UTC [h]',ytitle='Albedo',psym=3,charsize=cs,/xstyle
  for l=1,nl-1 do begin
    oplot,utf,nad[l,*]*dn[nind[l]]/(zen[l,*]*fac[flt,nind[l]]),color=40*l,psym=3
  endfor
  ...
  if attcorr then oplot,utf,fac[flt,nind[0]],psym=3
  ...
  plot,utc,alt,xr=uu,yr=[0,max(alt)],xtitle='UTC [h]',ytitle='Altitude [km] ',charsize=cs,title='ALTITUDE',psym=3,/xstyle

  ngeo = n_elements(alt)
  mxa  = max(alt)
  for i=0L,ngeo-2L do begin
  ...
  for i=0L,fix(ngeo,type=3)-2L do begin
    cl=fix(alt[i]/mxa*180)
...
if nav then begin
  ...
  for ii=0,n_elements(rt_wl)-1 do begin
    ...
    albedo[ii,*]=nspectra[i_z,*]/zspectra[i_z,*]  ; interpolate to common wavelength^M
  endfor
...
save, residual_ice,cod_ice,ref_ice, residual_liquid,cod_liquid,ref_liquid,cg4up,cg4dn,t_ambient, filename=dir+ll+date+ll+date+'_add.out'
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;; Begin Section of special plotting for specific Science flights
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
