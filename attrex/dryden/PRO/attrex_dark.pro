...
stop_checking = 0
    ...
    xsize=500
    ysize=300
    window,4,title='Dark Zenith',xsize=xsize,ysize=ysize,retain=2
    window,5,title='Dark Nadir',xsize=xsize,ysize=ysize,retain=2
...
; is closed (this is called dark cycle), the dark spectra are measured, and averaged.
    ...
    bignum=90000L       ; max of seconds (spectra), increase if needed
    darkmax=200                ; max of dark cycles, increase if needed
    ...
    for ind = 0, nf - 1 do begin ; go through all raw data files
        ...
        while not eof(lun) do begin
            ...
            nirxt[ctr] = float(1./dem - 273.)
                                ...
                                ; temperature(s)
                it = ((spect.it/2048.)*5.) - 5. ; Box temperature
...
window, 6, title='InGaAs darks time series', xsize=600,ysize=1000
!p.multi=[0,1,3] & loadct, 39,/silent & device,decomposed=0 &!p.BACKGROUND=255 & !p.color=0
plot, zir_hld[idrk,50],title='Zenith',/ylog,yrange=[10,17000],ytitle='Counts (log)',charsize=2.5
oplot,zir_hld[idrk,100],color=50
oplot,zir_hld[idrk,150],color=100
oplot,zir_hld[idrk,200],color=200
oplot,zir_hld[idrk,250],color=250
legend,['50','100','150','200','250'],textcolors=[0,50,100,200,250],box=0

plot, nir_hld[idrk,50],title='Nadir',yrange=[0,4000], ytitle='Counts',charsize=2.5
oplot,nir_hld[idrk,100],color=50
oplot,nir_hld[idrk,150],color=100
oplot,nir_hld[idrk,200],color=200
oplot,nir_hld[idrk,250],color=250
legend,['50','100','150','200','250'],textcolors=[0,50,100,200,250],box=0

plot, bx_hld[idrk], title='Temperatures',yrange=[-15,30], ytitle='Degrees C',charsize=2.5
oplot, nirtemp[idrk],color=50
oplot, zirtemp[idrk],color=100
oplot, nirxt[idrk],color=200
oplot, zirxt[idrk],color=250
legend,['Box','Nadir Spect','Zenith Spect','Nadir Housing','Zenith Housing'],textcolors=[0,50,100,200,250],box=0

filestr = '/home/leblanc/ATTREX/gh/'+date+'/dark_ex_'+date+'.png'
rgb=tvrd(true=1)
...
; Processing of all dark that have been measured
    ...
    for i=0,darkc do begin      ; trim darks
        ...
        if darklong[i] ge darkmin then begin
          ...
          long_enough=[long_enough,i]
        endif           
    endfor
    if n_elements(long_enough) gt 2 then begin
      long_enough=long_enough[1:*]
      ind0=ind0[long_enough]
      ind1=ind1[long_enough]
      darkc=n_elements(long_enough)-1
      darklong=darklong[long_enough]
      darktime=darktime[long_enough,*]
    endif
