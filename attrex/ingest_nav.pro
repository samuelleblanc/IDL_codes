...
;
; CALLING SEQUENCE:
;   ingest_nav,nav,date,pitchoff,rolloff,utc,lat,lon,alt,sza,iza,dc,rol,pit,hed,pre,tem,last_utc
...
;    - last_utc is the last utc time stamp that has been recorded, for use when using a single file that updates
; 
; OUTPUT:
;   see above
;
...
pro ingest_nav,nav,date,pitchoff,rolloff,utc,lat,lon,alt,sza,iza,dc,rol,pit,hed,pre,tem,last_utc 
newline = string(10B)
...
while not eof(lun) do begin
  readf, lun, line
  if strpos(line,'IWG1') eq -1 then continue ; when there isnt any data on the line
  line=strmid(line, strpos(line,'IWG1'))  ;search for the first occurence of the IWG1 string
  ...
  utc_temp = float(tmp0[3])+float(tmp0[4])/60. + float(tmp0[5])/3600. + plus
  if utc_temp le last_utc then continue  ;if the new line is not newer than the last recorded time stamp, then ignore this line
  if n_elements(tmp) ne 33 then continue
  utc[i] = float(tmp0[3])+float(tmp0[4])/60. + float(tmp0[5])/3600. + plus 
  lat[i] = float(tmp[2])  & lon[i] = float(tmp[3])  & alt[i] = float(tmp[5])
...
if i eq 0 then begin ; there is no new complete line in the file
  nan=!values.f_nan
  utc = nan & lat = nan & lon = nan & sza = nan & iza = nan
  dc  = nan & rol = nan & pit = nan & hed = nan & pre = nan
  y   = nan & m   = nan & d   = nan & alt = nan & tem = nan
  jul = nan
endif else begin
  utc = utc[0:i-1] & lat = lat[0:i-1] & lon = lon[0:i-1] & sza = sza[0:i-1] & iza = iza[0:i-1]
  dc  = dc[0:i-1]  & rol = rol[0:i-1] & pit = pit[0:i-1] & hed = hed[0:i-1] & pre = pre[0:i-1]
  y   = y[0:i-1]   & m   = m[0:i-1]   & d   = d[0:i-1]   & alt = alt[0:i-1] & tem = tem[0:i-1]
  jul = jul[0:i-1]
endelse
